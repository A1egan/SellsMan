...
def test_bundle_loader_and_public_config():
    ...
    for asset in (
        ...
        assert asset in workspace_core
...
def test_sync_never_clears_local_storage_and_uses_revision_rpc():
    sync = read("assets/cloud-sync.js")
    assert "localStorage.clear" not in sync
    ...
    assert "signed-out edits must" not in sync  # test prose must not leak into production bundle


def test_login_status_initialization_and_conflict_ui_contract():
...
def test_backup_v3_includes_work_tasks_and_v2_restore_defaults_empty():
    bootstrap = read("assets/cloud-sync-bootstrap.js")
...
def test_database_contract_is_owner_read_only_and_rpc_written():
    ...
    assert "revoke insert,update,delete" in sql.replace(" ", "")
